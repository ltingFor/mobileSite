    ...
    var pageUtil = {
        ...
        bindEvent: function() {
            ...
            j(".sortLi").on('click', function() {
                ...
                var sendData = sortType != 'tailor' ? { requesturl: '2xC2knBHKqVADnH5DKqj03tLidzCbhrn' } : { requesturl: 'mBI9zdBAgnR0RONCptStQjH%2FAQa4SD%2B1' }
                sendData.sortName = sortname;
                sendData.sortType = f ? 2 : 1;
            ...
            that.sesame_tansfer.bind('click', function() {
                ...
                var sendData = { requesturl: '2xC2knBHKqVADnH5DKqj03tLidzCbhrn' };
            ...
            if (!d.getListener('tailor_tansfer') || sortflag) {
                ...
                d.addListener('tailor_tansfer', {
                    ...
                    callback: function(data) {
                        ...
                        j('.buy-btn-tailor').unbind('click').bind("click", function(e) {
                            var o = j(this);
        ...
        get_sesame_trans_list: function(sendData, sortflag) { //芝麻开花转让专区列表
            var that = this;
            d.setListenerName('sesame_tansfer');
            console.log(!d.getListener('sesame_tansfer'))
            console.log('sortflag= ' + sortflag);
            console.log(d.droploader._scrollTop);
            d.droploader._scrollTop = 0;
            if (!d.getListener('sesame_tansfer') || sortflag) {
                ...
                d.addListener('sesame_tansfer', {
                    ...
                    callback: function(data) {
                        j('.buy-btn-sesame').bind("click", function(e) {
                            e.preventDefault();
                            var o = j(this);
                            var id = o.parents('li.mana-child-li').data('jump'),
                                status = o.data('status'),
                                proDetailExt = o.data('prodetailext');
                            var recordId = o.attr("data-recordId"),
                                accId = o.attr("data-accId"),
                                userId = o.attr("data-userId");
                            c.check_KH(function(data) {
                                if (data.code == '0000') {
                                    if (status == '0') { //出借
                                        window.location.href = PUBLIC.URL + "/static/mobileSite/html/transfer/transfer_invest.html?platform=" + platform + "&proDetailExt=" + encodeURIComponent(proDetailExt) + "&v=" + Math.random();
                                    } else { //取消出借
                                        l.alert1('确定取消转让吗？');
                                        l.setOkAction(function() {
                                            //var sd = $.base64.encode(JSON.stringify([{'prodStr':proDetailExt}]));
        ...
        creatHealper: function() {
            ...
            h.registerHelper('format1', function(amount, flag) {
                if (flag == 'yes') {
                    if (!+(amount)) {
                        ...
                        return +(amount) > 0 ? "+" + t.outputmoney_n(amount) : t.outputmoney_n(amount);
                    }
                } else {
                    return t.outputmoney_n(amount); //0.00
                }
            });
            ...
            Handlebars.registerHelper('jump', function(id, flag, transCreditId, accId, recordId, userId) {
                if (flag == 'tailorTrans') {
                    return PUBLIC.URL + "/static/mobileSite/html/transfer/tailor_transfer_detail.html?transCreditId=" + encodeURIComponent(transCreditId) + "&platform=" + platform + "&v=" + Math.random();
                } else if (flag == "sesameTrans") {
                    return PUBLIC.URL + "/static/mobileSite/html/transfer/transfer_detail.html?serviceParam=" + encodeURIComponent(id) + "&platform=" + platform + "&v=" + Math.random();
                }
            });
            ...
            Handlebars.registerHelper('checkBtnname', function(status) {
                return status == '0' ? '出借' : '取消转让';
            });
        },
        init: function() {
            var that = this;
            ...
            v.checkVip(function(data) { //检测是否有vip利率
                if (data.code == '0000') {
                    ...
                    that.get_sesame_trans_list({ requesturl: '2xC2knBHKqVADnH5DKqj03tLidzCbhrn' });
                }
            });
        }
    }
